...
class TestCohesiveChain(unittest.TestCase):
	...
	def setUp(self):
		...
		O.engines = [
		        ForceResetter(),
		        InsertionSortCollider([Bo1_ChainedCylinder_Aabb(), Bo1_Sphere_Aabb()]),
		        InteractionLoop(
	...
	def testMotion(self):
		...
		tv1 = -0.790576599652
		tp1 = -0.0483370740415
		tv2 = -0.000494271551993
		tp2 = -0.0611987315415
		tolerance = 10e-3
		O.run(100, True)
		v1 = O.bodies[0].state.vel[1]
		p1 = O.bodies[0].state.pos[1]
		#print v1,p1
		O.run(10000, True)
		v2 = O.bodies[0].state.vel[1]
		p2 = O.bodies[0].state.pos[1]
