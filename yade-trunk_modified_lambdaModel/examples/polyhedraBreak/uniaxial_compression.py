...
O.engines = [
        ...
        InteractionLoop(
                [Ig2_Facet_Polyhedra_PolyhedraGeom(),
                 ...
                 Ig2_Polyhedra_Polyhedra_PolyhedraGeom()], [Ip2_PolyhedraMat_PolyhedraMat_PolyhedraPhys()], [Law2_PolyhedraGeom_PolyhedraPhys_Volumetric()]
        ),
...
def addPlotData():
	...
	SigmaT = fT[2] / (sizeB * sizeB) * factorR
	uT = startPos - O.bodies[topmesh[0]].state.pos[2]
	SigmaL = fL[1] / (sizeB * sizeB) * factorR
	uL = sizeB - O.bodies[leftmesh[0]].state.pos[1]
	SigmaB = fB[0] / (sizeB * sizeB) * factorR
	uB = sizeB - O.bodies[backmesh[0]].state.pos[0]
	SigmaLT = 0.
	if (SigmaT):
		SigmaLT = SigmaL / SigmaT
	plot.addData(time=O.time, time2=O.time, Energy=utils.kineticEnergy(), SigmaT=SigmaT, uT=uT, SigmaL=SigmaL, uL=uL, SigmaB=SigmaB, uB=uB, SigmaLT=SigmaLT)
