...
def facetParallelepiped(center, extents, height, orientation=Quaternion((0, 1, 0), 0.0), wallMask=63, **kw):
	...
	if (height > extents[2]):
		raise RuntimeError("The height should be smaller or equal as extents[2]")
...
def facetCylinder(
	...
	return facetCylinderConeGenerator(
	        center=center,
	        radiusTop=radius,
	        height=height,
...
def facetSphere(center, radius, thetaResolution=8, phiResolution=8, returnElementMap=False, **kw):
	...
	if (thetaResolution < 3):
		raise RuntimeError("thetaResolution must be > 3")
	if (phiResolution < 3):
		raise RuntimeError("phiResolution must be > 3")
	...
	nodes.extend(
	        (
	                Vector3(c0 + r * math.cos(theta) * math.sin(phi), c1 + r * math.sin(theta) * math.sin(phi), c2 + r * math.cos(phi))
	                for phi in phis
	                for theta in thetas
	        )
	)
...
def facetCone(
	...
	"""
	# check zero dimentions
	if ((radiusBottom <= 0) and (radiusTop <= 0)):
		raise RuntimeError("The radiusBottom or radiusTop should have the positive value")
...
def facetPolygon(center, radiusOuter, orientation=Quaternion((0, 1, 0), 0.0), segmentsNumber=10, angleRange=None, radiusInner=0, **kw):
	...
	return facetPolygonHelixGenerator(
	        center=center,
...
def facetHelix(center, radiusOuter, pitch, orientation=Quaternion((0, 1, 0), 0.0), segmentsNumber=10, angleRange=None, radiusInner=0, **kw):
	...
	# check zero dimentions
	if (pitch <= 0):
		raise RuntimeError("The pitch should have the positive value")
	return facetPolygonHelixGenerator(
	        center=center,
	        radiusOuter=radiusOuter,
	        orientation=orientation,
...
def facetBunker(
	...
	# check zero dimentions
	if (dBunker <= 0):
		raise RuntimeError("The diameter dBunker should have the positive value")
	if (dOutput <= 0):
		raise RuntimeError("The diameter dOutput should have the positive value")
	if (hBunker < 0):
		raise RuntimeError("The height hBunker should have the positive or or zero")
	if (hOutput <= 0):
		raise RuntimeError("The height hOutput should have the positive value")
	if (hPipe < 0):
		raise RuntimeError("The height hPipe should have the positive value or zero")
	...
	ret += facetCone(
	        center=centerOutput,
	        radiusTop=dBunker / 2.0,
	        radiusBottom=dOutput / 2.0,
	        height=hOutput,
	        segmentsNumber=segmentsNumber,
	        wallMask=wallMask & 4,
	        angleRange=angleRange,
...
def facetPolygonHelixGenerator(center, radiusOuter, pitch=0, orientation=Quaternion((0, 1, 0), 0.0), segmentsNumber=10, angleRange=None, radiusInner=0, **kw):
	...
	if (radiusInner < 0):
		raise RuntimeError("The radiusInner should have the positive value or 0")
	if angleRange == None:
...
def facetCylinderConeGenerator(
        center,
        radiusTop,
        height,
        orientation=Quaternion((0, 1, 0), 0.0),
	...
	if (angleRange[1] < angleRange[0]):
		raise RuntimeError("angleRange[1] should be larger or equal angleRange[1]")
		...
		if (wallMask & 4):
			...
			if (abs(((angleRange[1] - angleRange[0])) >= math.pi)):
				if (radiusBottom != 0) and (radiusTop != 0):  #Cylinder
					pp = [
					        (ptsBottom[0][0], ptsBottom[0][1], -height / 2.0), (ptsBottom[1][0], ptsBottom[1][1], -height / 2.0),
					...
					pp = [
					        (ptsBottom[1][0], ptsBottom[1][1], -height / 2.0), (ptsTop[1][0], ptsTop[1][1], height / 2.0),
					        (ptsTop[0][0], ptsTop[0][1], height / 2.0)
					]
					pp = [orientation * p + center for p in pp]
					ret.append(utils.facet(pp, **kw))
					...
					pp = [
					        (0, 0, height / 2.0), (ptsBottom[0][0], ptsBottom[0][1], -height / 2.0),
				...
				if (radiusBottom != 0) and (radiusTop != 0):  #Cylinder
					pp = [
					        (ptsBottom[0][0], ptsBottom[0][1], -height / 2.0), (0, 0, -height / 2.0),
					        (ptsTop[0][0], ptsTop[0][1], height / 2.0)
					]
					pp = [orientation * p + center for p in pp]
					ret.append(utils.facet(pp, **kw))
