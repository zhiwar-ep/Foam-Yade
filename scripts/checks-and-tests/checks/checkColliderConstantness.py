...
for usePeriod in [True, False]:
	...
	O.run(500, True)
	results[usePeriod][0] = testedCollider.dumpBounds()
	O.run(1000, True)
	results[usePeriod][1] = testedCollider.dumpBounds()
				...
				for number in tupl:
					...
					if (loading):
						...
						if (abs(tmp - number) > 1e-8):
							if (yade.math.needsMpmathAtN(1) and (type(number) == int) and (abs(tmp - number) == 100)):
								...
								raise YadeCheckError(
								        "InsertionSortCollider check failed in file scripts/checks-and-tests/checks/data/checkColider.txt line: %d"
								        % lineCount
								)
					else:
						if (type(number) == int):
