...
// 	}
	...
	for (auto& yProc : yadeProcs) {
		if (yProc.numParticlesProc[localRank] > 0 ){
			...
			yProc.hydroForceBuff.resize(7*yProc.numParticles); //DasteXar changed from 6 to 7
...
void Foam::FoamYade::allocArrays(int sz, const std::shared_ptr<YadeProc>& yProc){
	...
	yProc->hydroForceBuff.resize(sz*7);  // DasteXar ( changed from 6 to 7)
...
void Foam::FoamYade::sendHydroForceYadeMPI(){
    ...
    for (auto& yProc : inCommProcs){
        for (auto & prt : yProc->foundParticles){
            ...
            // compute lambdaDot for this particle from its cell
            scalar ld = 0.0;
            const label cellI = prt->inCell;
            if (cellI >= 0 && cellI < lambdaDot.size())
            {
                ld = lambdaDot[cellI];
            }
            prt->lambdaDot = ld;
            ...
            // forces
            yProc->hydroForceBuff[base+0] = prt->hydroForce.x();
            yProc->hydroForceBuff[base+1] = prt->hydroForce.y();
            yProc->hydroForceBuff[base+2] = prt->hydroForce.z();
            // torques
            yProc->hydroForceBuff[base+3] = prt->hydroTorque.x();
            yProc->hydroForceBuff[base+4] = prt->hydroTorque.y();
            yProc->hydroForceBuff[base+5] = prt->hydroTorque.z();
            // lambdaDot
            yProc->hydroForceBuff[base+6] = ld;
        }
    }
    ...
    for (auto & yProc : inCommProcs){
        ...
        MPI_Send(&fBuff.front(), int(fBuff.size()),
                 MPI_DOUBLE, yProc->yRank, TAG_FORCE, interComm);
    }
}


/*    working but does not send lambda
void Foam::FoamYade::sendHydroForceYadeMPI(){
	// set forces in hydroForceBuff
	for (auto& yProc : inCommProcs){
		for (auto & prt : yProc->foundParticles){
			...
			yProc->hydroForceBuff[base+5] = prt->hydroTorque.z();
			//for lambdaDot
			yProc->hydroForceBuff[base+6] = prt->lambdaDot; // DasteXar 
		}
	}
	for (auto & yProc : inCommProcs){
		std::vector<double>& fBuff = yProc->hydroForceBuff;
...
void Foam::FoamYade::setParticleAction(double dt) {
	...
	//sendLambdaDotToYade();   // DasteXar: send per-particle lambdaDot values
