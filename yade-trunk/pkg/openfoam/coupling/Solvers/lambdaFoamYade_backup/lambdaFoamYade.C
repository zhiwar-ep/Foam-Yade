        ...
        for (const auto& procPtr : yadeCoupling.inCommProcs)
{
    ...
    for (const auto& partPtr : procPtr->foundParticles)
    {
        ...
        label cellI = mesh.findCell(partPtr->pos);
        if (cellI >= 0 && nParticles[cellI] > 0.5)
        {
            partPtr->lambdaDot = lambdaDot[cellI];
        }
    }
}
	//>
...
// Send (id, lambdaDot) to YADE

if (Pstream::master() && (runTime.timeIndex() % 10 == 0))
{
    if (!yadeCoupling.inCommProcs.empty() &&
        yadeCoupling.inCommProcs[0] &&
        !yadeCoupling.inCommProcs[0]->foundParticles.empty() &&
        yadeCoupling.inCommProcs[0]->foundParticles[0])
    {
        const auto& p = yadeCoupling.inCommProcs[0]->foundParticles[0];
        Info << "lambdaDot of particle (" << p->indx << "): " << p->lambdaDot << endl;
    }
}
