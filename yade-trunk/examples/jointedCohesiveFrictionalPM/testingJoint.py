...
O.engines = [
        ...
        InteractionLoop(
                [Ig2_Sphere_Sphere_ScGeom()], [Ip2_JCFpmMat_JCFpmMat_JCFpmPhys(cohesiveTresholdIteration=1)],
...
def dataCollector():
	plot.addData(
	        iterations=O.iter,
	        un=-(upperClump.state.pos[2] - z0),
	        Fz=O.forces.f(idUpperClump, sync=True)[2],
	        Ft=sqrt(pow(O.forces.f(idUpperClump, sync=True)[1], 2.) + pow(O.forces.f(idUpperClump, sync=True)[0], 2.)),
	        Fzinf=O.forces.f(idLowerClump, sync=True)[2],
	        uf=unbalancedForce(),
	        ec=kineticEnergy()
	)
