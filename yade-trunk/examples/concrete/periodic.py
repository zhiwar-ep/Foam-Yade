...
plot.plots = {
        'eps': ('sigma', 'sig1', 'sig2', None, 'eps', 'eps1', 'eps2'),
        'eps_': (
                'sigma',
                None,
                'relResid',
...
def stopIfDamaged():
	global mode
	if O.iter < 2 or 'sigma' not in plot.data:
	...
	if abs(sigma[-1] / extremum) < minMaxRatio or abs(strainer.strain[axis]) > 6e-3:
		if mode == 'tension' and doModes & 2:  # only if compression is enabled
			...
			import _thread
			_thread.start_new_thread(initTest, ())
			return
		else:
...
def addPlotData():
	yade.plot.addData(
	        t=O.time,
	        i=O.iter,
	        eps=strainer.strain[axis],
	        eps_=strainer.strain[axis],
	        sigma=strainer.stress[axis] + isoPrestress,
	        eps1=strainer.strain[ax1],
	        eps2=strainer.strain[ax2],
	        sig1=strainer.stress[ax1],
	        sig2=strainer.stress[ax2],
	        relResid=updater.avgRelResidual
	)
