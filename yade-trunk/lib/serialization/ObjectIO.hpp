...
struct ObjectIO {
	...
	template <class T, class oarchive> static void save(std::ostream& ofs, const std::string& objectTag, T& object)
	{
	...
	template <class T, class iarchive> static void load(std::istream& ifs, const std::string& objectTag, T& object)
	{
		std::locale default_locale(std::locale::classic(), new boost::archive::codecvt_null<char>);
