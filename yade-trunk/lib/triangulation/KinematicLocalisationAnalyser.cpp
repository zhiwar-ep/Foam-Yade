...
namespace CGT {
	...
	bool KinematicLocalisationAnalyser::DefToFile(const char* output_file_name)
	{
		...
		// renumber vertices to handle fictious or erased bodies
		vector<unsigned> altIds(TS0->maxId + 1, 0);
		unsigned         newId = 0;
		...
		for (; V_it != Tri.finite_vertices_end(); ++V_it) {
			...
			vtk.file << TS1->grain(V_it->info().id()).sphere.point() << endl; // get final positions in order to display with deformed shape
			altIds[V_it->info().id()] = newId++;
		}
		vtk.end_vertices();
		...
		for (; cell != Tri.finite_cells_end(); ++cell) {
			if (!cell->info().isFictious)
				vtk.write_cell(
				        altIds[cell->vertex(0)->info().id()],
				        altIds[cell->vertex(1)->info().id()],
				        altIds[cell->vertex(2)->info().id()],
				        altIds[cell->vertex(3)->info().id()]);
		}
		vtk.end_cells();
	...
	ofstream& KinematicLocalisationAnalyser::ContactDistributionToFile(ofstream& output_file)
	{
		...
		for (TriaxialState::ContactIterator cit = (*TS1).contacts_begin(); cit != cend; ++cit) {
			if ((*TS1).inside((*cit)->grain1->sphere.point()) && (*TS1).inside((*cit)->grain2->sphere.point())) {
				row[(int)(math::abs((*cit)->normal.z()) / DZ)].second += 2;
				nc1 += 2;
			} else {
				if ((*TS1).inside((*cit)->grain1->sphere.point()) || (*TS1).inside((*cit)->grain2->sphere.point())) {
					row[(int)(math::abs((*cit)->normal.z()) / DZ)].second += 1;
					++nc1;
				}
		...
		for (Edge_iterator ed_it = T.edges_begin(); ed_it != T.edges_end(); ed_it++) {
			if (!T.is_infinite(*ed_it)) {
				...
				if ((*TS1).inside(s.source()) && (*TS1).inside(s.target())) {
					v = s.to_vector();
					row[(int)(math::abs(v.z() / sqrt(s.squared_length())) / DZ)].second += 2;
					nv1 += 2;
				} else {
					if ((*TS1).inside(s.source()) || (*TS1).inside(s.target())) {
						v = s.to_vector();
						row[(int)(math::abs(v.z() / sqrt(s.squared_length())) / DZ)].second += 1;
						++nv1;
					} else
						++nv2;
				}
	...
	void KinematicLocalisationAnalyser::SetDisplacementIncrements(void)
	{
		...
		for (TriaxialState::GrainIterator git = TS1->grains_begin(); git != gend; ++git)
			if (git->id >= 0)
				git->translation = TS0->grain(git->id).translation = TS1->grain(git->id).sphere.point() - TS0->grain(git->id).sphere.point();
		consecutive = true;
	}
