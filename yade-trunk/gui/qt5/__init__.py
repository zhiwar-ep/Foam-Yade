...
elif os.path.exists(sphinxBuildDocPath + '/index.html'):
	sphinxPrefix = 'file://' + sphinxBuildDocPath
...
class ControllerClass(QWidget, Ui_Controller):
	...
	def addRenderers(self):
		self.displayCombo.addItem('OpenGLRenderer')
		afterSep = 1
	...
	def setTabActive(self, what):
		if what == 'simulation':
			...
			raise ValueErorr("No such tab: " + what)
	...
	def xyzSlot(self):
		self.setViewAxes((0, 0, -1), (0, 1, 0))

	def yzxSlot(self):
		self.setViewAxes((-1, 0, 0), (0, 0, 1))

	def zxySlot(self):
		self.setViewAxes((0, -1, 0), (1, 0, 0))
	...
	def refreshValues(self):
		...
		if len(self.iterTimes) == 0:
			self.iterTimes.append(rt)
			self.iterValues.append(iter)
			self.iterPerSec = 0  # update always for the first time
		elif rt - self.iterTimes[-1] > self.iterPerSecTimeout:  # update after a timeout
			if len(self.iterTimes) == 1:
				self.iterTimes.append(self.iterTimes[0])
				self.iterValues.append(self.iterValues[0])  # 2 values, first one is bogus
			self.iterTimes[0] = self.iterTimes[1]
			self.iterValues[0] = self.iterValues[1]
			self.iterTimes[1] = rt
			self.iterValues[1] = iter
		...
		if O.subStepping:
			subStep = O.subStep
			if subStep == -1:
				subStepInfo = u'→ <i>prologue</i>'
			elif subStep >= 0 and subStep < len(O.engines):
				e = O.engines[subStep]
				subStepInfo = u'→ %s' % (e.label if e.label else e.__class__.__name__)
			elif subStep == len(O.engines):
				subStepInfo = u'→ <i>epilogue</i>'
			else:
				raise RuntimeError("Invalid O.subStep value %d, should be ∈{-1,…,len(o.engines)}" % subStep)
