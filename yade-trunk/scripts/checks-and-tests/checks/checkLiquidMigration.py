...
if ('LIQMIGRATION' in features):
	...
	mat1 = O.materials.append(
	        ViscElCapMat(frictionAngle=fr, density=rho, Vb=VB, gamma=Gamma, theta=Theta, Capillar=True, CapillarType=CapillarType, tc=tc, en=en, et=et)
	)
	...
	O.run(3, True)
	if ((abs((O.interactions[id0, id1].phys.Vb - 0.03) / 0.03) > tolerance) or (abs((O.interactions[id0, id1].phys.Vb - 0.03) / 0.03) > tolerance)):
		raise YadeCheckError("error number 1")

	switchVel()
	O.run(5, True)
	if (
	        (abs((O.bodies[id0].state.Vf - 0.03) / 0.03) > tolerance) or (abs((O.bodies[id1].state.Vf - 0.04) / 0.04) > tolerance) or
	...
	O.run(5, True)
	if (
	        (abs((O.bodies[id0].state.Vf - 0.0465) / 0.0465) > tolerance) or (abs((O.bodies[id1].state.Vf - 0.0325) / 0.0325) > tolerance) or
